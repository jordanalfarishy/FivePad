...
| FR-1.15 | P1 | Semua | Menu penyuntingan teks berisi dua belas tindakan: Header, Sub header, Bold, Italic, Strike, List, Ordered List, To do, Quote, Code, Code Block, Link — ditambah sakelar tampilan FR-1.14 di baris paling atas. Di Android diakses lewat ikon `match_case` di bilah atas dan tampil sebagai **petak empat kolom**, bukan daftar bertumpuk: dua belas baris menghasilkan lembar setinggi hampir satu layar, dan lembar setinggi itu menutupi justru catatan yang sedang diformat. Di macOS lewat menu utama dengan pintasan papan tik. Setiap tindakan bersifat **membalik** — menerapkannya pada teks yang sudah memakainya justru mencabutnya, karena di tampilan teks biasa penandanya bahkan tidak terlihat untuk dihapus manual. |
| FR-1.15a | P1 | Semua | **Gaya boleh dipadukan.** Teks biasa, daftar, daftar bernomor, dan tugas menerima Bold, Italic, Strike, Code, dan Link; judul dan subjudul menerima Italic dan Strike; kutipan menerima Sub header, teks biasa, Bold, Italic, Strike, Code, dan Link. Penanda blok selalu ditulis lebih dulu — `> ## Judul`, bukan `## > Judul` — karena hanya urutan itu yang berarti "subjudul di dalam kutipan". |
